...
aggregated_expr_data <- function(cds, group_cells_by = "cell_type", gene_group_df = NULL,
  ...
  cluster_fraction_expressing_table <- tidyr::gather(
    cluster_fraction_expressing_table,
  ...
  cluster_expr_table <- tidyr::gather(
    cluster_expr_table, "cell_group",
...
plot_sub_contrast <- function(ccm,
  ...
  if (!is.null(selected_groups)) {
    partition_cell_groups <- cg_to_mg %>%
      filter(facet_group %in% selected_groups) %>%
      pull(cell_group)
...
filter_cds <- function(cds, ...) {
  cell_names <- as.data.frame(cds@colData) %>%
    filter(...) %>%
    rownames()
