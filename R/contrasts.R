...
estimate_abundances <- function(ccm, newdata, min_log_abund=-5){
  ...
  pred_out_tbl = tibble(cell_group=colnames(pred_out),
                        log_abund)
  #max_log_abundances,
  #min_log_abundances,
  #percent_max,
...
compare_abundances <- function(ccm, cond_x, cond_y){
  contrast_tbl = full_join(cond_x, cond_y, suffix = c("_x", "_y"), by="cell_group")
  contrast_tbl = contrast_tbl %>% mutate(delta_log_abund = log_abund_y - log_abund_x)
  return(contrast_tbl)
}
...
correlate_abundance_changes <- function(pln_model, cond_b_vs_a_tbl){
  ...
  cov_edges <- igraph::as_data_frame(cov_graph, what="edges") %>% filter(weight != 0.00)
  ...
  change_corr_tbl = left_join(change_corr_tbl, cond_b_vs_a_tbl %>% setNames(paste0('to_', names(.))), by=c("to"="to_cell_group")) #%>%
  #dplyr::rename(log_abund_x,
  #              to_delta_log_abund = delta_log_abund)
  change_corr_tbl = left_join(change_corr_tbl, cond_b_vs_a_tbl %>% setNames(paste0('from_', names(.))), by=c("from"="from_cell_group"))# %>%
