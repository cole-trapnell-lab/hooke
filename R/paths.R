...
calc_max_flow <- function(edges, source, target) {
  ...
  switch_dir_df <- new_pos_edge_coords_df %>%
    filter(flow < 0) %>%
    dplyr::rename(
      "from" = "to",
    ...
    mutate(flow = -flow)
  max_flow_edge_df <- rbind(
    filter(new_pos_edge_coords_df, flow >= 0),
    switch_dir_df
  ) %>%
    filter(flow > 0)
...
get_weighted_edges <- function(ccm,
  ...
  weighted_edge_df <- left_join(edges, dist_df, by = c("from", "to")) %>%
    mutate(
      x = abs(from_delta_log_abund - to_delta_log_abund),
      y = 1 / pcor,
      z = dist
    ) %>%
    mutate(
      x_z = (x - min(x)) / (max(x) - min(x)),
...
get_path <- function(ccm, cond_b_vs_a_tbl, q_value_threshold = 1.0) {
  ...
  edge_path <- neg_rec_edges %>%
    dplyr::mutate(shortest_path = purrr::map2(
      .f =
...
find_deg_path <- function(ccm,
  ...
  neg_rec_edges <- neg_rec_edges %>%
    dplyr::mutate(shortest_path = purrr::map2(
      .f = purrr::possibly(get_shortest_path, NA_real_),
      .x = from, .y = to,
      weighted_edges
    )) %>%
  ...
  if (adj_pairs) {
    ...
    neg_rec_edges <- neg_rec_edges %>%
      ...
      select(-weight) %>%
      distinct() %>%
      dplyr::mutate(states = purrr::map(
        .f = collect_transition_states,
        .x = from,
        .y = to
      ))
  } else {
    ...
    neg_rec_edges <- neg_rec_edges %>%
      dplyr::mutate(states = purrr::map(
        .f = collect_between_transition_states,
        .x = shortest_path
      ))
  }
  ...
  neg_rec_edges <- neg_rec_edges %>%
    dplyr::mutate(degs = purrr::map(
      .f = purrr::possibly(find_degs_between_states, NA_real_),
      .x = states,
      pb_cds
    ))
